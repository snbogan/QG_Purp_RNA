...
nrow(parental_QLFT_cutoff_df) # Without logFC cutoff = 3121 DEGs
...
Fig3C <- ggplot(data = ind_dfb,
  ...
  geom_smooth(method = "lm", formula = y~poly(x,2), 
              alpha = 0.25, color = "white", fill = "white") +
  theme_classic(base_size = 20) +
  scale_fill_gradient(high = "#ee765f", low = "#d8bcbb", guide = "none") +
  labs(y = "Absolute logFC",
...
for (i in seq_along(lme4qtls)) {
  
  # Extract point estimate of h² from VarProp
  h2_point <- VarProp(lme4qtls[[i]])[1, "prop"]
  
  # Bootstrap h² using bootMer (100 replicates)
  bootres <- bootMer(
    lme4qtls[[i]],
    FUN = function(fit) VarProp(fit)[1, "prop"],
    nsim = 100,
    use.u = FALSE,
    type = "parametric"
  )
  
  # Calculate standard error of h²
  h2_se <- sd(bootres$t)
  
  # Store in list as a small data frame
  evol_exp[[i]] <- data.frame(
    gene = names(lme4qtls)[i],
    h2 = h2_point,
    h2_se = h2_se
  )
}
...
lme4qtl_exp_evol_df <- bind_rows(evol_exp)
...
for (i in seq_along(pl_lme4s)) {
  
  # Extract point estimate of h² from VarProp
  h2_point <- VarProp(pl_lme4s[[i]])[1, "prop"]
  ...
  # Store in list as a small data frame
  evol_pl[[i]] <- data.frame(
    gene = names(pl_lme4s)[i],
    h2 = h2_point,
    h2_se = h2_se
  )
}

lme4qtl_pl_evol_df <- bind_rows(evol_pl)
lme4qtl_pl_evol_df$
...
save(lme4qtl_exp_evol_df, file = "lme4qtl_exp_evol_df.RData")
save(lme4qtl_pl_evol_df, file = "lme4qtl_pl_evol_df.RData")
...
load("lme4qtl_exp_evol_df.RData")
load("lme4qtl_pl_evol_df.RData")
...
load("~/Documents/GitHub/Cross_pHox/models_N.Rdata")
load("~/Documents/GitHub/Cross_pHox/models_U.Rdata")
...
colnames(lme4qtl_exp_evol_df)[1] <- "geneid"
colnames(lme4qtl_pl_evol_df)[1] <- "geneid"
...
bi_log_df$h2_beta <- bi_log_df$IA +
  0.000001
...
# h2 regression weighted by h2 error
h2_lm <- lm(h2 ~ as.factor(DE_bi) + poly(bod_ad,2) + poly(spi_ad,2) +
                        as.factor(DE_bi):poly(spi_ad,2) + as.factor(DE_bi):poly(spi_ad,2),
                      data = filter(bi_log_df, Trait == "DE"),
...
# Average h2 for DE
mean(lme4qtl_pl_evol_df$h2) # 0.267374

# Average se for pl h2
mean(lme4qtl_pl_evol_df$h2_se) # 0.115836
sd(lme4qtl_pl_evol_df$h2_se) # 0.04864695

# Average h2 for exp
mean(lme4qtl_exp_evol_df$h2) # 0.2229109

# Average se for exp h2
mean(lme4qtl_exp_evol_df$h2_se) # 0.1159003
sd(lme4qtl_exp_evol_df$h2_se) # 0.04862074

# ggplot(data = bi_log_df,
#        aes(x = bod_ad*-1, y = (h2_beta - 0.000001))) +
...
ggplot(data = df_4B,
  aes(x = p_eff, group = Trait)) +
  geom_rect(aes(ymin = 0, ymax = 1.0,
                xmin = (p_eff-1.1), xmax = (p_eff+1.1),
                fill = fit_landscape), alpha = 1) +
  geom_point(aes(y = h2), color = "white", size = 0.5, alpha = 0.1) +
  geom_errorbar(aes(y = h2, ymin = h2 - h2_se, ymax = h2 + h2_se), 
                width = 0, color = "white", size = 0.25, alpha = 0.1) +
  geom_smooth(aes(y = herit_bi), method = "glm", 
              method.args = list(family = "binomial"),
              fullrange = TRUE, color = "white", fill = "white") +
  theme_classic(base_size = 20, base_rect_size = 0) +
  theme(legend.position = "top") +
