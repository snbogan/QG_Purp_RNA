...
knitr::opts_knit$set(root.dir = '~/Documents/GitHub/QG_Purp_RNA/')
...
# Better describe factors for plot label
...
nrow(parental_QLFT_cutoff_df) # Without logFC cutoff
nrow(parental_QLFT_fc_cutoff_df) # With logFC cutoff
...
# Group data by dev vs. trans upwelling treatment
QLFT$Effect = factor(
  QLFT$Effect,
...
# Export Fig 5 as png
png("~/Documents/GitHub/QG_Purp_RNA/Fig2.png", units = "in", width = 12, 
    height = 7, 
    res = 600)
...
# Fit biomineralization selection gradient
...
# Fit body size selection gradient
...
# Generate sibship averages for plot
...
# Correct treatment group labels
...
# Generate biomineralization plasticity plot
...
# Generate biomineralization trait level plot
...
# Create body size plasticity plot
...
# Create body size trait level plot
...
# Assemble selection gradient panel
...
ggsave(Fig3ab,
       ...
       filename = "~/Documents/GitHub/QG_Purp_RNA/Fig3ab.png",
       bg = "white")

```

# Fit structural equation models of dev environment -> gene expression -> phenotype

Start with fitness proxy (percent abnormality)
...
Next, fit biominderalization (normalized spicule length) SEM
...
# Export Fig 3 as png
png("~/Documents/GitHub/QG_Purp_RNA/Fig3.png", units = "in", width = 12, 
    height = 12, 
    res = 600)
...
# Animal models estimating additive genetic variance and heritability
...
# Fit a test animal model using lme4qtl
...
# Separate data from dev treatments by sibship by treatment
tab_DevN <- filter(tab_QG_df, Dev == "N")
tab_DevU <- filter(tab_QG_df, Dev == "U")

# Merge tabular df with CPM data
Sib_FC_df <- merge(
...
# Construct high dimensional G matrices
...
# How many genes?
nrow(count(tab_pl_newf$geneid))
nrow(count(tab_pl_newf3$geneid))
...
write.csv(tab_pl_newf, 
          "~/Documents/GitHub/QG_Purp_RNA/tab_pl_newf.csv", 
          row.names = FALSE)

```

G matrix estimation for adaptive body size DE genes
...
save(models_N, file = "~/Documents/GitHub/QG_Purp_RNA/models_N.Rdata")
...
save(models_U, file = "~/Documents/GitHub/QG_Purp_RNA/models_U.Rdata")

load("~/Documents/GitHub/QG_Purp_RNA/models.Rdata")
...
Repeat G matrix estimation for adaptive biomineralization DE genes
...
#           "~/Documents/GitHub/QG_Purp_RNA/tab_pl_newf2.csv", 
#           row.names = FALSE)
...
save(models3_N, file = "~/Documents/GitHub/QG_Purp_RNA/models3_N.Rdata")
...
save(models3_U, file = "~/Documents/GitHub/QG_Purp_RNA/models3_U.Rdata")

load("~/Documents/GitHub/QG_Purp_RNA/models3.Rdata")
...
# Double check that dimensions are correct between input data and matrices
nrow(tab_pl_newf)
nrow(tab_pl_newf3)
...
# Tabularize matrices for further plotting and analysis

# Cluster the genetic correlation df
...
# Create genetic correlation heat map (not for paper)
...
# Combine genetic correlation data from different parental treatments
cov_pair_df <- rbind(
  merge(
    data.frame(
...
# Create plot for random permutation of matrix
...
png("~/Documents/GitHub/QG_Purp_RNA/g_panel.png", units = "in", width = 12, 
    height = 12, 
    res = 600)

g_panel

# Export cov_pair as png
png("~/Documents/GitHub/QG_Purp_RNA/cov_pairl.png", units = "in", width = 12, 
    height = 12, 
    res = 600)
...
# How many adaptive/maladaptive DEGs are heritable?
...
# Plot distributions of logCPM heritability and logFC heritability
...
colnames(lme4qtl_exp_evol_df) <- c("h2", "geneid", "Trait")
colnames(lme4qtl_pl_evol_df) <- c("h2", "geneid", "Trait")

GE_DE_h2_df <- rbind(lme4qtl_exp_evol_df, lme4qtl_pl_evol_df[,-c(4)])

GE_DE_h2_df$h2_beta <- GE_DE_IA_df$h2 + 0.000001

# Does h2 differ between logCPM and logFC?
GE_DE_h2_glm <- glm(h2_beta ~ Trait,
                       data = GE_DE_h2_df)

# GE is significantly less heritable than its plasticity
summary(GE_DE_h2_glm)
...
ggplot(data = plh2_v_abnorm,
  ...
  labs(x = "Fitness effect of DE (abnormality)", y = "DE h2")
...
ggplot(data = plh2_v_bod,
  ...
  geom_smooth(method = "loess", alpha = 0.25) +
  theme_classic(base_rect_size = 0) +
  labs(x = "Fitness effect of DE (body size)", y = "DE h2")
...
ggplot(data = filter(plh2_v_spi, geneid %in% sig_dev_meth_dfdm$column_label),
  ...
  theme_classic(base_rect_size = 0) +
  facet_wrap(~Dir) +
  labs(x = "Fitness effect of DE (spicule length)", y = "DE h2")
...
ggplot(data = filter(plh2_v_abnorm, geneid %in% sig_dev_meth_dfd$column_label),
  ...
  geom_boxplot() +
  theme_classic() +
  labs(y = expression("Differential Expression " ~ italic(h[2])),
...
# Multiply phenotypic effects by parental upwelling selection gradient
...
# Export Fig 5 as png
png("~/Documents/GitHub/QG_Purp_RNA/Fig4.png", units = "in", width = 7, 
    height = 12, 
    res = 600)
...
# Plot fitness landscape across DE and h2
...
# Generate fitness landscape plot
...
# Export Fig 5 as png
png("~/Documents/GitHub/QG_Purp_RNA/g1.png", units = "in", width = 7, 
    height = 7, 
    res = 600)
...
Next, create input data for go enrichment tests
...
rel_df$rel_h2 <- rel_df$rel_eff * rel_df$h2
rel_df$rel_logFC_h2 <- rel_df$rel_h2 * abs(rel_df$logFC)

rel_df$bod_h2 <- rel_df$bod_eff * rel_df$h2

rel_df$spi_h2 <- (-1*rel_df$spi_eff) * rel_df$h2

write.csv(
  data.frame(geneid = rel_df$geneid,
             rel_h2 = rel_df$rel_h2),
  row.names = FALSE,
...
write.csv(
  data.frame(geneid = rel_df$geneid,
             rel_logFC_h2 = rel_df$rel_logFC_h2),
  row.names = FALSE,
  "GO_MWU-master/rel_logFC_IA_GOMWU.csv")

write.csv(
  data.frame(geneid = rel_df$geneid,
             bod_h2 = rel_df$bod_h2),
  row.names = FALSE,
  "GO_MWU-master/bod_IA_GOMWU.csv")

write.csv(
  data.frame(geneid = rel_df$geneid,
             bod_h2 = rel_df$spi_h2),
  row.names = FALSE,
  "GO_MWU-master/spi_IA_GOMWU.csv")

```

Evaluate potential of transcript isoforms to affect GO ernichment among DE associated w/ adaptive plasticity

How many transcripts are isoforms of the same gene, and which? ... none among adaptive DEGs
...
# Count incidents of multiple isoforms across datasets
...
# What about DE associated with adaptive plasticity in biomineralization and boddy size?
...
setwd("~/Documents/GitHub/QG_Purp_RNA/GO_MWU-master/")
...
png("~/Documents/GitHub/QG_Purp_RNA/N_ped_plot.png", units = "in", width = 12, 
    height = 6, 
    res = 600)
...
png("~/Documents/GitHub/QG_Purp_RNA/U_ped_plot.png", units = "in", width = 12, 
    height = 6, 
    res = 600)
